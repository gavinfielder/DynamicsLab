 ...
 * 2014-11-02
 * 
 * Modified by gavinfielder@gmail.com to add non-delegate static constants
 *     > lines 299, 351-357, 508-514
 *     2018-04-17
 * 
 * Features:
...
namespace B83.ExpressionParser
{
    ...
    public class ExpressionParser
    {
        ...
        private Dictionary<string, double> staticConstants = new Dictionary<string, double>();
        ...
        public void AddStaticConst(string aName, double value)
        {
            if (staticConstants.ContainsKey(aName))
                staticConstants[aName] = value;
            else
                staticConstants.Add(aName, value);
        }
        ...
        IValue Parse(string aExpression)
        {
            ...
            foreach (var C in staticConstants)
            {
                if (aExpression == C.Key)
                {
